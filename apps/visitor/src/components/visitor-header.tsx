...
export default function VisitorHeader() {
  ...
  return (
    <header className="sticky top-0 z-50 bg-background border-b-[3px] border-border text-foreground font-mono">
      <div className="flex items-center justify-between px-4 py-2 max-w-3xl mx-auto gap-4">
        <Link
          to="/"
        ...
        <nav className="flex items-center gap-1 font-headline text-[12px] uppercase tracking-[1px]">
          {links.map(({ to, label, icon: Icon }) => (
            <Link
              key={to}
              to={to}
              className={`px-2.5 py-1.5 border-[2px] border-border transition-all whitespace-nowrap flex items-center gap-1 ${
                isActive(to)
          ...
          {isEntered && session?.displayId != null && (
            <span className="ml-1 px-2 py-1 border-[2px] border-border bg-muted text-[11px] font-black">
              #{session.displayId}
            </span>
          )}
        </nav>
